...
(defpackage :seq2seq
  (:use :cl :digikar-utilities :iterate :py4cl)
  (:shadowing-import-from :keras.layers :dense/class :lstm/class :input)
...
(defparameters
  ...
  (*epochs* 100 "Batch size for Number of epochs to train for.")
  (*latent-dim* 256 "Latent dimensionality of the encoding space.")
  (*num-samples* 10 "Number of samples to train on.")
  (*data-path* "fra.txt")
  (*validation-split* 0.2))

(defun define-compiled-model (num-encoder-tokens num-decoder-tokens latent-dim)

  (let* ((encoder-inputs (input :shape `(nil ,num-encoder-tokens)))
         ...
         (decoder-inputs (input :shape `(nil ,num-decoder-tokens)))
...
(defun train-model (model
  ...
  (pymethod-monitor model 'fit
                    (list (vector encoder-input-data decoder-input-data)
                          decoder-target-data
                          :batch-size batch-size
                          :epochs epochs
                          :validation-split validation-split)))

(let ((input-token-index-table (make-hash-table :size 100))    ; sets
      (target-token-index-table (make-hash-table :size 100))
...
target-texts"
    (iter (for line in-file data-path using #'read-line)
          ...
          (finally (return (list i input-texts target-texts)))))
  ...
  (defun process-texts-to-characters (input-texts target-texts)
    ...
    (iter outer
          ...
          (iter (for input-char in-string input-text)
                (unless (get-val input-token-index-table input-char 'hash-table)
                  (setf (get-val input-token-index-table input-char 'hash-table)
                        (in outer (next j)))
    ...
    (iter outer
          ...
          (iter (for target-char in-string target-text)
                (unless (get-val target-token-index-table target-char 'hash-table)
                  (setf (get-val target-token-index-table target-char 'hash-table)
                        (in outer (next j)))
  ...
  (defun input-text-to-3d-array (input-text)
    (let ((arr (make-array (list 1
      ...
      (iter (for char in-string input-text)
            (for i upfrom 0)
            (setf (get-val arr (list 0 i (get-val input-token-index-table char)))
                  1.0))
      arr))
  (defun target-text-to-3d-array (target-text)
    (let ((arr (make-array (list 1
      ...
      (iter (for char in-string target-text)
            (for i upfrom 0)
            (setf (get-val arr (list 0 i (get-val target-token-index-table char)))
                  1.0))
      arr))
        ...
        (iter (for input-text in input-texts)
              ...
              (iter (for id from 0)
                    ...
                    (setf (get-val encoder-input-data
                                   (list j id (get-val input-token-index-table
                                                       input-char)))
                          1.0))
              (iter (for id from 0)
                    (for target-char in-string target-text)
                    (setf (get-val decoder-input-data
                                   (list j id (get-val target-token-index-table
                                                       target-char)))
                          1.0)
                    (unless (first-iteration-p)
                      (setf (get-val decoder-target-data
                                     (list j (1- id) (get-val target-token-index-table
                                                              target-char)))
                            1.0))))
...
(defun restore-trained-model-for-inference
  ...
  (let* ((encoder-inputs (input :shape `(nil ,num-encoder-tokens)))
    ...
    (list (model encoder-inputs encoder-states)
          (let* ((decoder-c (input :shape `(,latent-dim)))
                 (decoder-h (input :shape `(,latent-dim)))
...
(defun define-decoder-model (trained-model num-decoder-tokens latent-dim)
  (let* ((decoder-inputs (input :shape `(nil ,num-decoder-tokens)))
         (decoder-h (input :shape `(,latent-dim)))
         (decoder-c (input :shape `(,latent-dim)))
